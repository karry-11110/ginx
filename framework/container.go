...
func (ginx *GinxContainer) make(key string, params []interface{}, forceNew bool) (interface{}, error) {
	...
	if forceNew {
		return ginx.newInstance(sp, params)
	}
	...
	// 容器中还未实例化，则进行一次实例化
	inst, err := ginx.newInstance(sp, nil)
	if err != nil {
		return nil, err
	}

	ginx.instances[key] = inst
	return inst, nil
}

// NameList 列出容器中所有服务提供者的字符串凭证
func (ginx *GinxContainer) NameList() []string {
	ret := []string{}
	for _, provider := range ginx.providers {
		name := provider.Name()
		ret = append(ret, name)
