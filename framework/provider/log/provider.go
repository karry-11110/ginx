...
import (
	"github.com/karry-11110/ginx/framework/provider/log/formatter"
	"io"
	"strings"

	"github.com/karry-11110/ginx/framework"
	"github.com/karry-11110/ginx/framework/contract"
	"github.com/karry-11110/ginx/framework/provider/log/services"
)

// GinxLogServiceProvider 服务提供者
type GinxLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *GinxLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewGinxConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewGinxSingleLog
	case "rotate":
		return services.NewGinxRotateLog
	case "console":
		return services.NewGinxConsoleLog
	case "custom":
		return services.NewGinxCustomLog
	default:
		return services.NewGinxConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *GinxLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *GinxLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *GinxLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *GinxLogServiceProvider) Name() string {
